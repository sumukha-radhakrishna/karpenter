...
var _ = Describe("CEL/Validation", func() {
	...
	Context("Disruption", func() {
		...
		It("should succeed when setting consolidateAfter with consolidationPolicy=WhenEmpty", func() {
			...
			nodePool.Spec.Disruption.ConsolidationPolicy = lo.ToPtr(ConsolidationPolicyWhenEmpty)
			Expect(env.Client.Create(ctx, nodePool)).To(Succeed())
		})
		It("should succeed when setting consolidateAfter with consolidationPolicy=WhenUnderutilized", func() {
			nodePool.Spec.Disruption.ConsolidateAfter = lo.ToPtr(MustParseNillableDuration("30s"))
			nodePool.Spec.Disruption.ConsolidationPolicy = lo.ToPtr(ConsolidationPolicyWhenEmptyOrUnderutilized)
			Expect(env.Client.Create(ctx, nodePool)).To(Succeed())
		})
	...
	Context("Replicas", func() {
		Context("Valid Replicas Values", func() {
			...
			It("should succeed when replicas is set to a large value", func() {
				nodePool.Spec.Replicas = lo.ToPtr(int32(1000))
				Expect(env.Client.Create(ctx, nodePool)).To(Succeed())
				Expect(nodePool.RuntimeValidate(ctx)).To(Succeed())
			})
		...
		Context("Invalid Replicas Values", func() {
			It("should fail when replicas is set to a negative value", func() {
				nodePool.Spec.Replicas = lo.ToPtr(int32(-100))
				Expect(env.Client.Create(ctx, nodePool)).ToNot(Succeed())
			})
		})
		...
		Context("Incompatible Fields", func() {
			...
			It("should fail runtime validation when replicas and non-default consolidateAfter are both set", func() {
				nodePool.Spec.Replicas = lo.ToPtr(int32(5))
				nodePool.Spec.Disruption.ConsolidateAfter = lo.ToPtr(MustParseNillableDuration("10m"))
				Expect(env.Client.Create(ctx, nodePool)).To(Succeed())
				Expect(nodePool.RuntimeValidate(ctx)).ToNot(Succeed())
			})
			It("should fail runtime validation when replicas and default consolidateAfter (0s) are both set", func() {
				nodePool.Spec.Replicas = lo.ToPtr(int32(5))
				nodePool.Spec.Disruption.ConsolidateAfter = lo.ToPtr(MustParseNillableDuration("0s"))
				Expect(env.Client.Create(ctx, nodePool)).To(Succeed())
				Expect(nodePool.RuntimeValidate(ctx)).ToNot(Succeed())
			})
		})
		...
		Context("Compatible Fields", func() {
			It("should succeed  when replicas and 'Never' consolidateAfter are both set", func() {
				nodePool.Spec.Replicas = lo.ToPtr(int32(5))
				nodePool.Spec.Disruption.ConsolidateAfter = lo.ToPtr(MustParseNillableDuration("Never"))
				Expect(env.Client.Create(ctx, nodePool)).To(Succeed())
				Expect(nodePool.RuntimeValidate(ctx)).To(Succeed())
		...
		Context("Update Scenarios", func() {
			It("should succeed when adding replicas to a NodePool without incompatible fields", func() {
				...
				updatedNodePool.Spec.Replicas = lo.ToPtr(int32(3))
			...
			It("should succeed when removing replicas from a NodePool", func() {
				...
				nodePool.Spec.Replicas = lo.ToPtr(int32(3))
				Expect(env.Client.Create(ctx, nodePool)).To(Succeed())

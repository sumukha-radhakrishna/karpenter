...
func (c *Controller) Reconcile(ctx context.Context, np *v1.NodePool) (reconcile.Result, error) {
	...
	for range lo.FromPtr(np.Spec.Replicas) - nodes.Value() {
		...
		nct.InstanceTypeOptions, _ = scheduling.FilterInstanceTypesByRequirements(its, nct.Requirements, corev1.ResourceList{}, corev1.ResourceList{}, corev1.ResourceList{})

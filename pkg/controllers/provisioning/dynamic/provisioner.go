...
func (p *Provisioner) Reconcile(ctx context.Context) (result reconcile.Result, err error) {
	...
	if _, err = p.ncProvisioner.CreateNodeClaims(ctx, results.NewNodeClaims, provisioning.WithReason(metrics.ProvisionedReason), provisioning.RecordPodNomination, provisioning.IgnoreStaticNodeClaims); err != nil {
		return reconcile.Result{}, err
	}
	return reconcile.Result{RequeueAfter: singleton.RequeueImmediately}, nil
}
